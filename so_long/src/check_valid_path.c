...
/*   Updated: 2023/11/20 16:43:12 by gigardin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	allocate_flag_grid(t_data *game, t_map *seen_flag)
{
	...
	seen_flag->grid_matrix = malloc((seen_flag->rows) * sizeof(char *));
	if (seen_flag->grid_matrix == NULL)
		handle_error(1, "Error!\n", game);
	while (y < game->map->rows)
	{
		seen_flag->grid_matrix[y] = malloc(seen_flag->columns * sizeof(char));
